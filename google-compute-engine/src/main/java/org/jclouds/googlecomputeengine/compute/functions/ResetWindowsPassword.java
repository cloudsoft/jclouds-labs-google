   ...
   public String apply(Map<String, ?> params) {
      ...
      try {
       ...
       try {
          ...
          boolean passwordRetrieved = Predicates2.retry(new Predicate<InstanceApi>() {
             public boolean apply(InstanceApi instanceApi) {
                String serialPortContents = instanceApi.getSerialPortOutput(instance.get().name(), 4).contents();
          ...
          }, 10 * 60, 30, TimeUnit.SECONDS).apply(instanceApi); // Notice that timeoutDuration should be less than EXPIRE_DURATION
